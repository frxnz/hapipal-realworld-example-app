...
module.exports = class ArticleService extends Schmervice.Service {
    ...
    async find({ tag, author, favorited, limit, offset }, txn) {
        ...
        const query = Article.query(txn).withGraphFetched('[author, tags, favoritedBy]');
    ...
    async fetchArticlesByUser(userId, articles, transaction) {

        const { Article } = this.server.models();

        const results = await Article.fetchGraph(articles, `[
            tags,
            favoritedBy(currentUser) as favoritedBy,
            author.[
                followedBy(currentUser) as following
            ]
        ]`, { transaction })
            .modifiers({
                currentUser: (builder) => builder.where('Users.id', userId),
    ...
    async feed(currentUserId, { limit, offset }, txn) {
        ...
        const query = Article.query(txn)
            .withGraphFetched('[author, tags, favoritedBy]')
